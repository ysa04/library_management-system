                        ...
                        <div class="card book-card">
                            <div class="card-body">
                                ...
                                if (isset($_GET['id'])) {
                                    ...
                                    $query = "SELECT id, title, author, summary, book_count, publication_year,shelf, stat, image_data, dewey_number, sub_category, book_description, sub_description FROM books WHERE id = ?";
                                    ...
                                    if ($result->num_rows == 1) {
                                        ...
                                        echo "<p class='card-text'><strong>Remaining number of books:</strong> " . $row['book_count'] . "</p>";
                                        echo "<p class='card-text'><strong>Status:</strong> " . $row['stat'] . "</p>";
                                        echo "<p class='card-text'><strong>Shelf Number:</strong> " . $row['shelf'] . "</p>";
                                        ...
                                        echo "<p class='card-text'><strong>Book LCC:</strong> " . $row['dewey_number'] . $row['sub_category'] . "</p>";
                                        echo "</div>";
                                        ...
                                        $disabled = $row['book_count'] <= 0 ? 'disabled' : '';

                                        // Add student_id to the borrow buttons
                                        echo "<button class='btn btn-primary borrow-btn me-2' data-book-id='" . htmlspecialchars($row['id']) . "' data-borrow-type='inside' data-student-id='" . htmlspecialchars($student_id) . "' data-book-title='" . htmlspecialchars($row['title']) . "' $disabled>Borrow Inside</button>";
                                        echo "<button class='btn btn-secondary borrow-btn' data-book-id='" . htmlspecialchars($row['id']) . "' data-borrow-type='outside' data-student-id='" . htmlspecialchars($student_id) . "' data-book-title='" . htmlspecialchars($row['title']) . "' $disabled>Borrow Outside</button>";
        ...
        button.addEventListener('click', function() {
            ...
            if (confirmation) {
                ...
                fetch('borrow.php', {
                    ...
                    body: JSON.stringify({ action: 'borrow', bookId, borrowType, studentId, bookTitle })
                })
